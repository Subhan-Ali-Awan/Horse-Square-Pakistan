...
import { Send, CheckCircle, Clock, User, ShieldCheck, MessageSquare } from 'lucide-react';
...
export const QueryChatModal = ({ isOpen, onClose, query, currentUser, onQueryUpdated }) => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 text-[10px] font-black uppercase px-3 py-1 rounded-full border ${map[status] || map.new}`}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="" maxWidth="max-w-2xl">
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-200 pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-extrabold text-slate-900 text-base">{query.subject || 'General Inquiry'}</h2>
            </div>
        ...
        <div className="max-h-[380px] overflow-y-auto space-y-4 pr-2 p-1 bg-slate-50/50 rounded-2xl border border-slate-100">
          ...
          <div className={`flex flex-col ${isAdmin ? 'items-start' : 'items-end'} space-y-1`}>
            <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 px-1">
              ...
              <span>{query.name} {!isAdmin ? '(You - Original Query)' : '(User Inquiry)'}</span>
              <span>•</span>
                ...
                <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500 px-1">
                  {isFromAdmin ? (
                    <>
                      ...
                      <span className="text-slate-900 font-extrabold">{reply.senderName || 'Support Admin'} {isMe ? '(You)' : ''}</span>
                      <span>•</span>
                      <span className="font-normal text-slate-400">
                    ...
                    <>
                      <User className="w-3.5 h-3.5 text-amber-600" />
                      <span className="text-slate-900 font-extrabold">{reply.senderName || query.name} {isMe ? '(You)' : ''}</span>
                      <span>•</span>
                      <span className="font-normal text-slate-400">
                        {new Date(reply.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
        ...
        {/* Reply Input Box */}
        {query.status === 'resolved' ? (
          <div className="bg-emerald-50 text-emerald-900 p-4 rounded-2xl border border-emerald-200 text-center flex items-center justify-center gap-2 text-xs font-bold">
            <CheckCircle className="w-4 h-4 text-emerald-600" />
            <span>This query has been marked as resolved. You can still read the entire conversation thread above.</span>
          </div>
        ) : (
          <form onSubmit={handleSendReply} className="flex items-end gap-3 pt-2">
            ...
            <button
              ...
              className="py-3 px-5 bg-[#0F172A] hover:bg-[#1E293B] text-white font-bold rounded-xl text-xs transition shadow flex items-center gap-1.5 shrink-0 cursor-pointer disabled:opacity-50"
            >
